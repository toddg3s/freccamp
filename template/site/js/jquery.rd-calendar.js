...
(function ($, window, document, undefined) {
    ...
    function RDCalendar(element, options) {
        ...
        this.$element.data('currentDate', new Date());
    ...
    RDCalendar.Defaults = {
        days: ['S', 'M', 'T', 'W', 'T', 'F', 'S'],
        days_full: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thuthday', 'Friday', 'Saturday'],
        ...
        classes: {
            ...
            table_events: 'rdc-table_events-count',
    ...
    RDCalendar.prototype.watch = function () {
        ...
        self.$element
            ...
            .end()
            .delegate('.' + self.options.classes.table_event, 'click', function () {
                self.$element
                    .find('.' + self.options.classes.event + ', .' + self.options.classes.table_event)
                    .removeClass('active');
    ...
    RDCalendar.prototype.getTodayDay = function () {
        return this.options.days_full[this.$element.data('todayDate').getDay()];
    };
        ...
        for (var i = 0; i < 7; i++) {
            ...
            for (var j = 0; j < 7; j++) {
                ...
                if (currentDate.valueOf() in this.evt) {
                    currentDay.addClass(opt.classes.table_event);
                }


                $week.append($('<td/>').append(currentDay.data('date', currentDate.valueOf())));
            }
